"""Test for promote admin endpoint when user is already an admin."""
...
@pytest.mark.asyncio
async def test_promote_admin_already_admin_direct(
    test_setup: TestClient, test_db: AsyncSession
) -> None:
    """Test promoting a user who is already an admin via direct function call."""
    ...
    with patch("src.util.rest_util.sio.emit", new_callable=AsyncMock) as mock_emit:
        response = await promote_admin.promote_admin(
            promote_request2, admin_auth, test_db
        )
