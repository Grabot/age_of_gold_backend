...
from age_of_gold_worker.age_of_gold_worker.util.util import worker_upload_media
...
def test_worker_upload_media(
    mock_get_s3_client: MagicMock, mock_s3_client: MagicMock
) -> None:
    """Test that worker_upload_media uploads the image to S3 correctly."""
    ...
    worker_upload_media(test_avatar_bytes, test_bucket, test_s3_key)
...
@patch("age_of_gold_worker.age_of_gold_worker.util.s3_client.get_s3_client")
def test_worker_upload_media_empty_data(
    mock_get_s3_client: MagicMock, mock_s3_client: MagicMock
) -> None:
    """Test that worker_upload_media handles empty data."""
    mock_get_s3_client.return_value = mock_s3_client

    worker_upload_media(b"", "test-bucket", "test-key.png")
