...
async def test_engine_creation(mock_settings: MockerFixture) -> None:
    ...
    assert engine_async.pool.size() == 5
    assert engine_async.pool._max_overflow == 10
    assert engine_async.pool._pre_ping is True
    assert engine_async.pool._recycle == 3600
