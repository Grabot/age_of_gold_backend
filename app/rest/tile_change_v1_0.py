...
class MapRest(Resource):
    ...
    def post(self):
        ...
        if auth_token == '':
            return response_failed("back to login")
        ...
        if not user.can_change_tile_type():
            return response_failed("not allowed")

        tile_q = json_data["q"]
        tile_r = json_data["r"]
        tile_type = json_data["type"]
        if not tile_q or not tile_r or not tile_type or not tile_q.isdigit() or not tile_r.lstrip("-").isdigit() or not tile_type.lstrip("-").isdigit():
            return response_failed("error occurred")
        ...
        emit("change_tile_type_success", tile.serialize_full, room=room, namespace='/api/v1.0/sock')
...
api = Api(app_api)
api.add_resource(MapRest, '/api/v1.0/tile/change', endpoint='change_tile')
