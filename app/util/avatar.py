...
class Plane:
    def __init__(self, points: List[Point], colour: str) -> None:
        self.points = points
        ...
        self.lines: List[Line] = [line1, line2, line3, line4]
        self.colour = colour

    def get_all_lines(self) -> List[Line]:
        return self.lines

    def get_line(self, line_choice: int) -> Line:
...
def get_angle_lines(line_1: Line, line_2: Line) -> float:
...
def angle_slopes(slope_1: float, slope_2: float) -> float:
...
def point_on_line(_line: Line, _point: Point) -> bool:
...
def get_length(start_point: Point, end_point: Point) -> float:
...
def add_square_clean(
    ...
    while attempts < 100:
        ...
        colour_index -= _index
        _index += 1
        plane_choice = random.randint(_index, len(_planes) - 1 + _index)
        plane_choice -= _index
        _index += 1
        plane = _planes[plane_choice]
        line_choice = random.randint(_index, 3 + _index)
        line_choice -= _index
        _index += 1
        ...
        _index += 1
        angle_choice = random.randint(_index, len(angles) + _index)
        angle_choice -= _index
        angle_choice -= 1
        _index += 1
...
def background_square_clean(_width: int, _height: int, _index: int) -> Plane:
    ...
    left_point: Point = (point_a, 0)
    point_c = point_a * -1
    right_point: Point = (point_c, 0)
    ...
    bottom_point: Point = (0, point_b)
    point_d = point_b * -1
    top_point: Point = (0, point_d)
    colour_index = random.randint(_index, len(colours) - 1 + _index)
    colour_index -= _index
    _index += 1
...
def generate_avatar(file_name: str, file_path: str) -> None:
    ...
    width = 252
    height = 252
    background_plane = background_square_clean(width, height, index)
    index += 1
    ...
    for plane in planes:
        ...
        plane_colour = tuple(
            int(plane.colour.lstrip("#")[i : i + 2], 16)  # noqa: E203
            for i in (0, 2, 4)
        )
